...
from constants import SAVE_PATH, PATIENCE, MAX_EPOCH, MAX_GRAD_NORM, DEVICE, CKPT
...
def load(param, device):
    ...
    with open(res_path, 'w') as f:
        ...
        for ckpt in checkpoints:
            ...
            log_ckpt(dev_evaluator, dev_loader, 'Dev', ckpt, writer)
...
def create_trainer(model, optimizer, scheduler, loss_fn, max_grad_norm, device):
    ...
    def _update(engine, batch):
        ...
        y_pred = model(*x)
        losses = [loss_fn(pred, y) for pred in y_pred]
        weights = [model.w_cat, model.w_t, model.w_v, model.w_a]
        loss = sum([a*b for a, b in zip(losses, weights)])
        loss.backward()
